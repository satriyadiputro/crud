    ...
    try{
        ...
        if(e.name === 'PrismaClientKnownRequestError'){
            if(message.includes("Unique constraint failed on the fields")){
                const matches = message.match(/\(([^)]+)\)/g)[0].replace(/`|\(|\)/g, '')
                return { status: 409, message:"Conflict", data: "Duplicate content. '"+matches+"' is exist" }
            } else {
                return { status: 400, message:"Bad_Request", data: e.message }
            }
            
        } else {
            return { status: 500, message:"Internal Server Error", data:message.replace(/[\n]/g, "") }
        }
